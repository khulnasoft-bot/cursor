...
export class EnhancedCompletionService {
    ...
    async getCompletion(request: CompletionRequest): Promise<CompletionResult> {
        ...
        try {
            ...
            console.warn(
                'AI service completion failed, falling back to worker:',
                error
            )
    ...
    private buildCompletionPrompt(
        ...
        if (
            ...
            context.importedSymbols.length > 0
        ) {
    ...
    private parseCompletionResponse(
        ...
        let nextEditLocation:
            { line: number; column: number; reason: string } | undefined
        ...
        if (
            completion.includes('function ') ||
            completion.includes('class ') ||
            completion.includes('=>')
        ) {
        ...
        return {
            completion,
            suggestedImports:
                suggestedImports.length > 0 ? suggestedImports : undefined,
            nextEditLocation,
            confidence: 0.85,
        }
    }
    ...
    async predictNextEditLocation(
        ...
        for (
            let i = cursorLine;
            i < Math.min(lines.length, cursorLine + 50);
            i++
        ) {
