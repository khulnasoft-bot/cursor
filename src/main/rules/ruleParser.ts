...
export class RuleParser {
    ...
    async loadRulesFromDirectory(directory: string): Promise<ParsedRules> {
        ...
        try {
            ...
            try {
                ...
                log.info(
                    'No .cursor/rules directory found, using default rules'
                )
            ...
            log.info(
                `Loaded ${this.ruleSets.size} rule sets with ${activeRules.length} active rules`
            )
    ...
    private async loadRuleFile(filePath: string): Promise<void> {
        try {
            ...
            if (!validation.valid) {
                this.errors.push(
                    `Validation failed for ${filePath}: ${validation.error}`
                )
                return
            }
            ...
            this.errors.push(
                `Failed to parse rule file ${filePath}: ${error instanceof Error ? error.message : 'Unknown error'}`
            )
        }
    }
    ...
    private validateRuleSet(ruleSet: RuleSet): {
        valid: boolean
        error?: string
    } {
        if (!ruleSet.name) {
            ...
            if (ruleSet.inherits && ruleSet.inherits.length > 0) {
                ...
                for (const parentName of ruleSet.inherits) {
                    ...
                    if (parentSet) {
                        ...
                        this.errors.push(
                            `Parent rule set not found: ${parentName}`
                        )
                    }
                }
                ...
                if (ruleSet.overrides) {
                    for (const [ruleId, override] of Object.entries(
                        ruleSet.overrides
                    )) {
                        const ruleIndex = inheritedRules.findIndex(
                            (r) => r.id === ruleId
                        )
                        if (ruleIndex !== -1) {
                            inheritedRules[ruleIndex] = {
                                ...inheritedRules[ruleIndex],
                                ...override,
                            }
                        }
