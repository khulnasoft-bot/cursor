...
export function setupRulesIpcs(): void {
    ...
    ipcMain.handle(
        ...
        async (_event: IpcMainInvokeEvent, code: string, filePath: string) => {
            try {
                const result = await ruleService.applyRulesToCode(
                    code,
                    filePath
                )
    ...
    // Get active rules
    ipcMain.handle('rules-get-active', async () => {
        try {
    ...
    // Get all rule sets
    ipcMain.handle('rules-get-rule-sets', async () => {
        try {
    ...
    // Get statistics
    ipcMain.handle('rules-get-statistics', async () => {
        try {
