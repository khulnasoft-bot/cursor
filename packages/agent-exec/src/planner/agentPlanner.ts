...
export class AgentPlanner {
    ...
    setToolRegistry(toolRegistry: ToolRegistry): void {
        ...
        this.logger.info('Tool registry set for agent planner')
    }
    ...
    async executePlan(planId: string, onProgress?: (step: AgentStep, progress: number) => void): Promise<AgentPlan> {
        ...
        for (let i = 0; i < plan.steps.length; i++) {
            ...
            if (!this.checkDependencies(step, plan.steps)) {
                step.status = 'skipped'
                this.logger.info(`Step ${step.id} skipped due to unmet dependencies`)
                continue
            }
            ...
            try {
                ...
                this.logger.error(`Step ${step.id} error:`, error)
        ...
        plan.completedAt = new Date()
        this.logger.info(`Plan ${planId} completed successfully`)
    ...
    createGoal(description: string, priority: GoalPriority = 'medium', dependencies: string[] = []): AgentGoal {
        ...
        this.logger.info(`Created goal ${goalId}: ${description}`)
    ...
    completeGoal(goalId: string): void {
        ...
        if (goal) {
            ...
            this.logger.info(`Completed goal ${goalId}`)
        }
    }
    ...
    reset(): void {
        ...
        this.planCounter = 0
        this.goalCounter = 0
        this.logger.info('Reset agent planner')
    }
}
...
export function getAgentPlanner(logger?: Logger): AgentPlanner {
    if (!agentPlanner) {
        agentPlanner = new AgentPlanner(logger)
    }
    return agentPlanner
}

export function destroyAgentPlanner(): void {
    if (agentPlanner) {
        agentPlanner.reset()
        agentPlanner = null
    }
...
export function createAgentPlanner(logger?: Logger): AgentPlanner {
    return new AgentPlanner(logger)
}