...
export class SemanticIndexer {
    ...
    async indexFile(
        ...
        for (const chunk of chunks) {
            ...
            try {
                ...
                this.logger.warn(
                    `Failed to generate embedding for chunk ${codeChunk.id}:`,
                    error
                )
    ...
    async indexDirectory(
        ...
        this.logger.info(
            `Starting semantic indexing for directory: ${directoryPath}`
        )
        ...
        this.logger.info(
            `Semantic indexing complete. Total chunks: ${this.index.totalChunks}`
        )
    }
    ...
    private chunkContent(
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (
                currentLength + lineLength > (this.config.chunkSize || 500) &&
                ...
                const overlapLines = Math.floor(
                    (this.config.chunkOverlap || 50) /
                        ((this.config.chunkSize || 500) /
    ...
    getChunksByFile(filePath: string): CodeChunk[] {
        return Array.from(this.index.chunks.values()).filter(
...
export function getSemanticIndexer(
...
export function createSemanticIndexer(
    config?: IndexerConfig,
    logger?: Logger
): SemanticIndexer {
